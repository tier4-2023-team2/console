...
import { AxesHelper, BoxGeometry, CylinderGeometry, DoubleSide, Vector3, } from 'three';
...
export default function Vehicle() {
  ...
  const axes_ref = React.useRef(null);

  const AxisHelper = ({ color, direction, length }) => {
    ...
    const lineGeometry = new THREE.BufferGeometry().setFromPoints([new Vector3(), normalizedDirection.multiplyScalar(length - arrowHeadLength)]);
  ...
  const MyAxes = () => {
    const axis_length = 3;
    return (<>
      <AxisHelper color="red" direction={new Vector3(0, 0, 1)} length={axis_length} />
      <AxisHelper color="green" direction={new Vector3(1, 0, 0)} length={axis_length} />
      <AxisHelper color="blue" direction={new Vector3(0, 1, 0)} length={axis_length} />
    </>)
  }
